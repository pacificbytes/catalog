...
export default function ImageCarousel({ images, productName }: ImageCarouselProps) {
	...
	const changeImage = useCallback((newIndex: number) => {
		...
		// Load the new image and adjacent images
		const newLoadedImages = new Set(loadedImages);
		newLoadedImages.add(newIndex);
		
		// Also load adjacent images for smoother experience
		if (newIndex > 0) newLoadedImages.add(newIndex - 1);
		if (newIndex < images.length - 1) newLoadedImages.add(newIndex + 1);
		
		setLoadedImages(newLoadedImages);
		setSelectedIndex(newIndex);
	}, [selectedIndex, loadedImages, images.length]);

	// Debug logging
	useEffect(() => {
		console.log('ImageCarousel - loadedImages:', Array.from(loadedImages));
		console.log('ImageCarousel - selectedIndex:', selectedIndex);
		console.log('ImageCarousel - images:', images.map(img => ({ id: img.id, url: img.url })));
	}, [loadedImages, selectedIndex, images]);
		...
		<div className="space-y-4">
			...
			>
				...
				{images.length > 1 && (
					<>
						<button
							onClick={() => changeImage(selectedIndex === 0 ? images.length - 1 : selectedIndex - 1)}
						...
						<button
							onClick={() => changeImage(selectedIndex === images.length - 1 ? 0 : selectedIndex + 1)}
